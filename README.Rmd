...
For interval estimation with unweighted survival data, `cifcurve()` mirrors the CI options of `survival::survfit()` (`error="greenwood"` and `"tsiatis"`). For weighted survival data, `cifcurve()` adds valid CIs and SEs of Xie and Liu (2005), and for competing risk data, `error="delta"` and `"aalen"` options. The defaults in `cifmodeling` are chosen from external and internal simulation evidence; they should be sensible for most applications, while still allowing experts to override them when needed. 
