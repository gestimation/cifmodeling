...
[![CRAN status](https://www.r-pkg.org/badges/version/cifmodeling)](https://CRAN.R-project.org/package=cifmodeling)
[![CRAN downloads per month](https://cranlogs.r-pkg.org/badges/last-month/cifmodeling)](https://cranlogs.r-pkg.org/badges/last-month/cifmodeling)
[![CRAN downloads total](https://cranlogs.r-pkg.org/badges/grand-total/cifmodeling)](https://cranlogs.r-pkg.org/badges/grand-total/cifmodeling)

<!-- badges: end -->

```{r setup, eval=FALSE, include=FALSE, warning=FALSE, message=FALSE}
#install.packages(c("cifmodeling", "Rcpp", "nleqslv", "boot", "ggplot2", "ggsurvfit", "patchwork", "modelsummary"))

if (!requireNamespace("cifmodeling", quietly = TRUE)) {
  stop("Package 'cifmodeling' is not available. Please install it from CRAN first.")
}
library(cifmodeling)
library(nleqslv)
library(boot)
...
It provides a unified, high-level interface for survival and competing risks analysis, combining nonparametric estimation, hypothesis testing, regression modeling, and visualization. It is centered around four tightly connected functions:
...
- `ciftest()` compares survival or CIF curves using log-rank, Gray, robust score, augmented score, or multiple-direction tests.
...
```{r example01, fig.cap="Aalen-Johansen cumulative incidence curves from cifplot()", warning=FALSE, message=FALSE}
library(cifmodeling)
...
`ciftest()` uses the same formula interface. Its default is log-rank for a survival outcome and the augmented score test for a competing-risk outcome. Short aliases are available for interactive use: `"L"`/`"LR"`/`"log-rank"`, `"G"`, and `"A"`/`"aug"`/`"augmentation"`. The `"early"` and `"late"` presets choose the outcome-specific test with Fleming--Harrington weights, while `"multiple"` (or `"multi"`/`"m"`) combines the default directions `(2, 0)`, `(0, 2)`, and `(0, 0)`. Optional lower truncation of nuisance probabilities is controlled by `prob.truncation`.

```{r ciftest-example, eval=FALSE}
ciftest(
...
```{r example02, fig.cap="Cumulative incidence curves with competing-risk marks", warning=FALSE, message=FALSE}
data(diabetes.complications)
...
```{r example03, fig.cap="Adjusted cumulative incidence curves with CIs based on influence functions", warning=FALSE, message=FALSE}
...
```{r example04, fig.cap="Estimation of risk ratios for both competing events at 8 years using polyreg()", warning=FALSE, message=FALSE}
...
```{r example05, fig.cap="Visualization of risk ratios at 2, 4, 6 and 8 years using polyreg() and modelplot()", warning=FALSE, message=FALSE}
...
- **Curve-comparison tests**: `ciftest()` provides log-rank, Gray, robust score, augmented score, and rank-adaptive multiple-direction tests through the same formula syntax.
...
**cifmodeling** includes an extensive test suite built with **testthat**, which checks the numerical accuracy and graphical consistency of the core estimation, testing, modeling, and plotting functions, including `ciftest()`. Numerical results are compared against related functions in **survival**, **cmprsk**, and **mets** where independent reference implementations are available.
