...
-   `code.event2` Integer code of the competing event (default `2`).
-   `code.censoring` Integer code of censoring (default `0`).
-   `code.events` Optional numeric length-3 vector `c(event1, event2, censoring)`. When supplied, it overrides `code.event1`, `code.event2`, and `code.censoring`(primarily used when `printEachEvent = TRUE`).
-   `error` Character specifying variance type used internally. For `"SURVIVAL"` typically `"greenwood"`. For `"COMPETING-RISK"` pass options supported by `calculateAalenDeltaSE()` (`"aalen"`, `"delta"`, `"none"`).
-   `conf.type` Character transformation for CI on the probability scale (default `"arcsine-square root"`).
-   `conf.int` numeric two-sided confidence level (default `0.95`).
