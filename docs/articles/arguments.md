...
- `return_if` Logical. When `TRUE` and `engine = "calculateAJ_Rcpp"`,
  the influence function is also computed and returned (default
  `FALSE`).
...
- `boot.multiplier` Character string specifying the wild bootstrap
  weight distribution. One of `"rademacher"`, `"mammen"` or `"gaussian"`
  (default `"rademacher"`).
