...
- [`ciftest()`](https://gestimation.github.io/cifmodeling/reference/ciftest.md)
  compares survival or CIF curves using log-rank, Gray, robust score,
  augmented score, or multiple-direction tests.
...
cifplot()](reference/figures/README-example01-1.png)
...
[`ciftest()`](https://gestimation.github.io/cifmodeling/reference/ciftest.md)
uses the same formula interface. Its default is log-rank for a survival
outcome and the augmented score test for a competing-risk outcome. Short
aliases are available for interactive use: `"L"`/`"LR"`/`"log-rank"`,
`"G"`, and `"A"`/`"aug"`/`"augmentation"`. The `"early"` and `"late"`
presets choose the outcome-specific test with Fleming–Harrington
weights, while `"multiple"` (or `"multi"`/`"m"`) combines the default
directions `(2, 0)`, `(0, 2)`, and `(0, 0)`. Optional lower truncation
of nuisance probabilities is controlled by `prob.truncation`.
...
Then,
[`cifplot()`](https://gestimation.github.io/cifmodeling/reference/cifplot.md)
is used to generate the figure. The `label.y`, `label.x`, `label.strata`
and `limit.x` arguments are also used to customize **the labels and axis
limits**.
...
marks](reference/figures/README-example02-1.png)
...
IPWs from [`CBPS()`](https://rdrr.io/pkg/CBPS/man/CBPS.html) and draw
adjusted CIFs. It is important to note that in IPW analysis, the SEs
output in the unweighted analysis are not necessarily valid. Among the
SE methods selectable in
[`cifplot()`](https://gestimation.github.io/cifmodeling/reference/cifplot.md)
...
functions](reference/figures/README-example03-1.png)
...
We then fit
[`polyreg()`](https://gestimation.github.io/cifmodeling/reference/polyreg.md)
to estimate risk ratios for diabetic retinopathy and macrovascular
complications, in a coherent joint model of all cause-specific CIFs at 8
years.
...
[`modelplot()`](https://modelsummary.com/man/modelplot.html) to
visualize how risk ratios evolve over follow-up. With
generics-Compatible objects,
[`polyreg()`](https://gestimation.github.io/cifmodeling/reference/polyreg.md)
is integrated naturally with the broader `broom/modelsummary` ecosystem.
For publication-ready tables, you can pass `polyreg` objects directly to
...
![Visualization of risk ratios at 2, 4, 6 and 8 years using polyreg()
and modelplot()](reference/figures/README-example05-1.png)
...
- **Curve-comparison tests**:
  [`ciftest()`](https://gestimation.github.io/cifmodeling/reference/ciftest.md)
  provides log-rank, Gray, robust score, augmented score, and
  rank-adaptive multiple-direction tests through the same formula
  syntax.
...
| Gray test | Yes (`ciftest`) | No (only log-rank via `survdiff`) | Yes (`cuminc`) | No | [`tidycmprsk::glance()`](https://generics.r-lib.org/reference/glance.html) + `add_pvalue()` |
...
functions, including
[`ciftest()`](https://gestimation.github.io/cifmodeling/reference/ciftest.md).
Numerical results are compared against related functions in
**survival**, **cmprsk**, and **mets** where independent reference
