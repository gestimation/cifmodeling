...
-   `add.ci` Logical add `add_confidence_interval()` to plot. It calls `geom_ribbon()` (default `TRUE`).
...
-   `add.ci`, `add.censor.mark`, `add.competing.risk.mark`, `add.intercurrent.event.mark`, `add.quantile` Optional logical vectors/lists per panel to toggle features in `cifplot()`. If `NULL`, sensible defaults are used (CI/Censor on; others off).
