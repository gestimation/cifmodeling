...
For the initial illustration, unadjusted analysis focusing on cumulative incidence of diabetic retinopathy (event 1) and macrovascular complications (event 2) at 8 years of follow-up is demonstrated. To visualize each covariate separately when multiple strata are supplied, set `printEachVar = TRUE`. Each variable on the right-hand side is plotted in its own panel, and the layout can be controlled with `rows.columns.panel`. The figure below contrasts the cumulative incidence curves of diabetic retinopathy for the quartiles `fruitq` and a binary exposure `fruitq1`, low (Q1) and high (Q2 to 4) intake of fruit, generated by `cifplot()`. The `addConfidenceInterval=TRUE` argument adds confidence intervals to the plot. This helps visualize the statistical uncertainty of estimated probabilities across exposure levels. When using numeric variables for stratification, discretize them beforehand with `cut()` or `factor()`. The labels of x-axis (Time) and y-axis (Cumulative incidence) in these panels are default labels. 
...
```{r example5-1, warning=FALSE, message=FALSE}
data(diabetes.complications)
diabetes.complications$d <- as.integer(diabetes.complications$epsilon>0)
diabetes.complications$fruitq1 <- ifelse(
  diabetes.complications$fruitq == "Q1","Q1","Q2 to Q4"
...
```{r example5-2, warning=FALSE, message=FALSE}
...
```{r example5-3, warning=FALSE, message=FALSE}
# Adjust font size
...
```{r example5-4, warning=FALSE, message=FALSE}
...
cifplot(Event(t,d) ~ fruitq1,
        ...
        limits.x              = c(0, 8),
        breaks.x              = 0:8,
        use_coord_cartesian   = TRUE) 
```

### Panel mode

```{r example5-5, warning=FALSE, message=FALSE}
# Each stratification variable
cifplot(Event(t,epsilon) ~ fruitq + fruitq1,
        data                  = diabetes.complications,
        outcome.type          = "COMPETING-RISK",
        printEachVar          = TRUE)
        
# Event 1 vs event 2 in competing risks data
cifplot(Event(t,epsilon) ~ fruitq1,
        data                  = diabetes.complications,
        outcome.type          = "COMPETING-RISK",
        printEachEvent        = TRUE)

# Event vs censoring in survival data
cifplot(Event(t,d) ~ fruitq1,
        data                  = diabetes.complications,
        outcome.type          = "SURVIVAL",
        printCensoring        = TRUE)
...
-   `linewidth` Optional numeric specifying the line width of curves.
