...
-   In `cifcurve()` the RHS of the formula defines the grouping variable(s) whose curves are estimated; weights (if any) are supplied separately. There is no dedicated exposure argument because all strata are handled through the formula term that is turned into a factor for plotting and estimation.
...
- The `error` argument determines how SEs are computed. For survival data (`outcome.type = "survival"`) it maps to Greenwood (default) or Tsiatis estimators, while for competing-risks data (`outcome.type = "competing-risk"`) `"aalen"` reproduces the classic Aalen standard errors and `"delta"` implements the delta method. The SEs for weighted Kaplan-Meier estimators are calculated using the formulas by Zie and Liu (2005). 
...
-   `statistic` – which uncertainty summaries to display. Examples: `statistic = "p.value"` for p-values, `statistic = "conf.int"` for Wald CIs, `statistic = "t"` for t statistics, `and statistic = "z"` for z statistics, instead of SEs (default).
