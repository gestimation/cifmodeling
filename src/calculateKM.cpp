...
struct RecW {
 ...
 Rcpp::List calculateAJ_Rcpp(
   ...
   bool all_w_one = true;
   for (int i = 0; i < N; ++i) {
     if (std::fabs(W[i] - 1.0) > 0.0) { all_w_one = false; break; }
   }
   ...
   std::vector<int> guniq_vec; guniq_vec.reserve(N);
   std::unordered_set<int> seen;
   seen.reserve(N);

   for (int i=0;i<N;++i){
     int g = G[i];
     if (seen.insert(g).second) guniq_vec.push_back(g);
   }
   ...
   std::vector<double> combined_times, combined_surv, combined_n_risk, combined_n_event, combined_n_censor, combined_std_err, combined_std_err_cif;
   std::vector<double> combined_high, combined_low;
   std::vector<int>    combined_n_stratum, combined_u_stratum;
   std::vector<double> combined_cif_se_if, combined_cif_se_aalen, combined_cif_se_delta;
   ...
   for (int kg=0; kg<K; ++kg){
     ...
     // ALL unique times & weighted counts
     ...
     // risk set on ALL (suffix)
     std::vector<double> Y_all(Uall, 0.0);
     double suf=0.0;
     for (int j=Uall-1;j>=0;--j){ suf += w_total_all[j]; Y_all[j]=suf; }

     // === [C-2] Unweighted risk set (suffix of counts) ===
     std::vector<double> Y_unw_all(Uall, 0.0);
     {
       double suf_unw = 0.0;
       for (int j = Uall - 1; j >= 0; --j) {
         suf_unw += cnt_total_all[j];
         Y_unw_all[j] = suf_unw;
       }
     }

     // ANY-event indices & cause-1 indices (on ALL)
     std::vector<int> ev_any_idx, ev_c1_idx;
     for (int j=0;j<Uall;++j){
       if (w_event_all[j] > 0.0) ev_any_idx.push_back(j);
       if (w_event1_all[j] > 0.0) ev_c1_idx.push_back(j);
     }
     const int M_any = (int)ev_any_idx.size();
     const int M_c1  = (int)ev_c1_idx.size();

     // KM on ANY grid: S and var(GW/TS)
     std::vector<double> S_any(M_any, 1.0), varGW_any(M_any, 0.0), varTS_any(M_any, 0.0);
     std::vector<double> seLOG_GW_any(M_any, 0.0), seLOG_TS_any(M_any, 0.0);
     double accGW=0.0, accTS=0.0;
     for (int m=0;m<M_any;++m){
       ...
       if (dj>0.0 && Yj>0.0){
         if (Yj > dj) accGW += dj / (Yj*(Yj-dj)); else accGW = std::numeric_limits<double>::infinity();
         accTS += dj / (Yj*Yj);
       }
       double S2 = S_any[m] * S_any[m];
       varGW_any[m] = S2 * accGW;
       varTS_any[m] = S2 * accTS;
       seLOG_GW_any[m] = (R_finite(accGW) ? std::sqrt(accGW) : R_PosInf);
       seLOG_TS_any[m] = (R_finite(accTS) ? std::sqrt(accTS) : R_PosInf);
       }

     // Map KM S/SE to ALL times (carry forward)
     ...
     for (int j=0;j<Uall;++j){
       ...
       if (c>0){
         S_all[j] = S_any[c-1];
         double v = varGW_any[c-1];
         if (error=="tsiatis") v = varTS_any[c-1];
         SE_all[j] = (R_finite(v) && v>=0.0) ? std::sqrt(v) : R_PosInf;
       } else { S_all[j]=1.0; SE_all[j]=0.0; }
     }

     std::vector<double> SE_log_surv(Uall, 0.0);
     std::vector<double> SE_surv(Uall, 0.0);
     double acc = 0.0;
     for (int j = 0; j < Uall; ++j) {
     ...
     // AJ for cause-1 (on ANY grid)
     std::vector<double> dl_any(M_any,0.0), dl_c1(M_any,0.0), aj1_any(M_any,0.0);
     {
       for (int m=0;m<M_any;++m){
         int j = ev_any_idx[m];
         double Yj = Y_all[j], dj = w_event_all[j];
       ...
       for (int m=0;m<M_any;++m){
         int j = ev_any_idx[m];
         double Yj = Y_all[j];
         if (is_c1_at_all[j] && Yj>0.0){
           dl_c1[m] = w_event1_all[j] / Yj;
         } else dl_c1[m]=0.0;
       }
     ...
     // AJ IF per individual on ANY grid -> then expand to ALL grid
     NumericMatrix IF_AJ_all(n_g, Uall);
     if (return_if){
       ...
       // expand IF to ALL times (carry forward)
       for (int j=0;j<Uall;++j){
         int c = all2any[j];
         if (c==0) {
           for (int r=0;r<n_g;++r) IF_AJ_all(r,j) = 0.0;
         } else {
           for (int r=0;r<n_g;++r) IF_AJ_all(r,j) = IF_AJ_any(r, c-1);
         }
       }
     }

     // CIF variance by IF aggregation on ALL grid
     std::vector<double> cif_var_if(Uall, 0.0);
     if (return_if){
       for (int r=0;r<n_g;++r){
         for (int j=0;j<Uall;++j){
           double v = IF_AJ_all(r,j);
           cif_var_if[j] += v*v;
         }
       }
     }

     // --- Aalen / Delta variance (cause-1) on cause-1 grid, then carry-forward to ALL ---
     ...
     for (int u=0; u<M_c1; ++u){
       ...
       c1_Y[u]    = Y_all[j];
       // F1 at that time (map ANY->cause1)
       int count_any_before = 0;
       // find ANY index equal to this ALL index j:
     ...
     // carry-forward Aalen/Delta to ALL grid
     ...
     std::vector<double> se_cif_if(Uall, 0.0), se_cif_aalen(Uall, 0.0), se_cif_delta(Uall, 0.0);
     for (int j=0;j<Uall;++j){
       int c = all2c1[j];
       se_cif_if[j]    = std::sqrt( std::max(0.0, (return_if ? cif_var_if[j] : 0.0)) );
       se_cif_aalen[j] = (c>0 ? std::sqrt(std::max(0.0, var_aalen[c-1])) : 0.0);
       se_cif_delta[j] = (c>0 ? std::sqrt(std::max(0.0, var_delta[c-1])) : 0.0);
     }

     // high/low for S(t) via log–log (uses SE_all chosen above)
     ...
     if (CONF != "none"){
       for (int j=0;j<Uall;++j){
         ...
         double SEj = SE_cif[j];
//         double S = clamp01(S_all[j], 1e-15);
//         double seS = SEsurv_all[j];
     ...
     IF_AJ_output[kg] = return_if ? IF_AJ_all : Rcpp::NumericMatrix(0,0);
   }

   std::string out_type   = "kaplan-meier";
   std::string out_method = "Kaplan-Meier";
   if (error=="aalen" || error=="delta" || error=="if"){ out_type="aalen-johansen"; out_method="Aalen-Johansen"; }

   SEXP strata_out, strata_levels_out_sexp;
   if (K==1){ strata_out = Rcpp::IntegerVector(0); strata_levels_out_sexp = Rcpp::IntegerVector(0); }
   else      { strata_out = wrap(combined_u_stratum); strata_levels_out_sexp = wrap(strata_levels_out); }
